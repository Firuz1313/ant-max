...
interface DataContextType {
  ...
  createTVInterface: (data: Partial<TVInterface>) => Promise<TVInterface>;
  updateTVInterface: (
    id: string,
    data: Partial<TVInterface>,
  ) => Promise<TVInterface>;
  deleteTVInterface: (id: string) => Promise<void>;
  getTVInterface: (id: string) => TVInterface | undefined;
...
export const DataProvider: React.FC<DataProviderProps> = ({ children }) => {
  ...
  const [tvInterfaces, setTVInterfaces] = useState<TVInterface[]>(() => {
    const stored = localStorage.getItem("ant-support-tv-interfaces");
    return stored ? JSON.parse(stored) : [];
  });
  ...
  useEffect(() => {
    localStorage.setItem(
      "ant-support-tv-interfaces",
      JSON.stringify(tvInterfaces),
    );
  }, [tvInterfaces]);
  ...
  // TV Interface operations
  const createTVInterface = useCallback(
    async (data: Partial<TVInterface>): Promise<TVInterface> => {
      try {
        setLoading((prev) => ({ ...prev, tvInterfaces: true }));

        const newTVInterface: TVInterface = {
          id: generateId(),
          name: data.name || "",
          description: data.description || "",
        ...
        setTVInterfaces((prev) => [...prev, newTVInterface]);
        logChange("tv_interface", newTVInterface.id, "create", {
          new: newTVInterface,
        });

        return newTVInterface;
      } catch (error) {
        console.error("Error creating TV interface:", error);
        throw error;
      } finally {
        setLoading((prev) => ({ ...prev, tvInterfaces: false }));
      }
    },
    [],
  );

  const updateTVInterface = useCallback(
    async (id: string, data: Partial<TVInterface>): Promise<TVInterface> => {
      try {
        setLoading((prev) => ({ ...prev, tvInterfaces: true }));

        const oldTVInterface = tvInterfaces.find((t) => t.id === id);
        if (!oldTVInterface) {
          throw new Error("TV Interface not found");
        }

        const updatedTVInterface: TVInterface = {
          ...oldTVInterface,
          ...data,
          updatedAt: createTimestamp(),
        };
        ...
        setTVInterfaces((prev) =>
          prev.map((t) => (t.id === id ? updatedTVInterface : t)),
        );
        logChange("tv_interface", id, "update", {
          old: oldTVInterface,
          new: updatedTVInterface,
        });

        return updatedTVInterface;
      } catch (error) {
  ...
  const deleteTVInterface = useCallback(
    async (id: string): Promise<void> => {
      try {
        setLoading((prev) => ({ ...prev, tvInterfaces: true }));

        const tvInterface = tvInterfaces.find((t) => t.id === id);
        if (!tvInterface) {
          throw new Error("TV Interface not found");
        }

        setTVInterfaces((prev) => prev.filter((t) => t.id !== id));
        logChange("tv_interface", id, "delete", { old: tvInterface });
      } catch (error) {
  ...
  const getTVInterface = useCallback(
    (id: string): TVInterface | undefined => {
      return tvInterfaces.find((t) => t.id === id);
    },
    [tvInterfaces],
  );
  ...
  const exportData = useCallback(
    async (options: ExportOptions): Promise<{ downloadUrl: string }> => {
      ...
      if (options.entities.includes("tv_interfaces")) {
        dataToExport.tvInterfaces = tvInterfaces;
      }
    ...
    [devices, problems, steps, remotes, tvInterfaces],
  );
