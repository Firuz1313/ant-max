...
const testProblems = [
  ...
  },
  {
    ...
    description: 'Каналы переключаются очень медленно или зависают при переключении',
    category: 'moderate',
    icon: 'Clock',
    color: 'from-yellow-500 to-yellow-600',
  ...
  },
  {
    ...
    description: 'Появляется сообщение об ошибке авторизации при попытке просмотра каналов',
    category: 'critical',
    icon: 'AlertTriangle',
    color: 'from-red-500 to-red-600',
...
const testSteps = [
  // Шаги для проблемы "Нет сигнала" OpenBox
  {
    ...
    instruction: 'Проверьте подключение HDMI или AV кабеля между п��иставкой и телевизором. Убедитесь, что кабель плотно вставлен в разъемы.',
    estimated_time: 60,
  ...
  },
  {
    ...
    description: 'Убедитесь, что ��а телевизоре выбран правильный источник сигнала',
    instruction: 'Нажмите кнопку "Input" или "Source" на пульте телевизора и выберите вход, к которому подключена приставка (HDMI1, HDMI2, AV и т.д.).',
    estimated_time: 45,
  ...
  // Шаги для проблемы "Пульт не работает" OpenBox
  {
    ...
    description: 'Убедитесь, что батарейки установлены правильно и не разряжены',
    instruction: 'Откройте крышку батарейного отсека на пульте. Проверьте, что батарейки установлены согласно полярности (+/-). Попробуйте заменить батарейки на новые.',
    estimated_time: 120,
  ...
  },
  {
    ...
    instruction: 'Направьте пульт на приставку и нажмите любую кнопку. Если есть смартфон с камерой, посмотрите через камеру на ИК-диод пульта при нажатии кнопки - должен мигать красный свет.',
    estimated_time: 60,
  ...
  // Шаги для проблемы "Медленная загрузка каналов" UCLAN
  {
    ...
    description: 'Выполните перезагрузку для очистки временных файло��',
    instruction: 'Отключите приставку от сети питания на 10 секунд, затем включите снова. Дождитесь полной загрузки системы.',
    estimated_time: 180,
  ...
  },
  {
    ...
    instruction: 'Зайдите в Настройки → Система → Память → Очистить кэш. Подтвердите действие и дождитесь завершения процесса.',
    estimated_time: 120,
...
export const seedDatabase = async () => {
  try {
    ...
    await transaction(async (client) => {
      ...
      // Добавление нескольких тесто��ых сессий
      console.log('📊 Добавление тестовых сессий...');
      for (let i = 0; i < 10; i++) {
        const randomDevice = testDevices[Math.floor(Math.random() * testDevices.length)];
        const deviceProblems = testProblems.filter(p => p.device_id === randomDevice.id);
        
        if (deviceProblems.length > 0) {
          const randomProblem = deviceProblems[Math.floor(Math.random() * deviceProblems.length)];
          ...
          const sessionData = {
            ...
            completed_steps: success ? Math.floor(Math.random() * 5) + 1 : Math.floor(Math.random() * 3),
            success,
