...
export async function testConnection() {
  ...
  try {
    ...
    console.log(`📋 Версия PostgreSQL: ${result.rows[0].postgres_version.split(' ')[0]}`);
...
export async function searchText(searchTerm, tables = ['problems', 'devices', 'diagnostic_steps']) {
  try {
    const searchResults = {};
